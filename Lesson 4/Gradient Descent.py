...
np.random.seed(0)
torch.manual_seed(0)
x_train = torch.linspace(-1, 4, 1000).unsqueeze(1)
#y_train = torch.pow((1 - x_train), 3)  # Using torch.pow for element-wise exponentiation
...
def train_model_with_real_time_plot(model, x_train, y_train, num_epochs=10000, learning_rate=0.22):
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_title(f'Supervised Learning Fit')
    ax.legend()
    ...
    def update(frame, model, x_train, line, epoch_text):
        ...
        if frame % 1 == 0:  # Update the plot every 1 epochs
            line.set_data(x_train, y_pred.detach().numpy()[:, 0])
            epoch = frame * 10
